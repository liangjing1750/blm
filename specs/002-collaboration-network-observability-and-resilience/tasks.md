...
- [x] T6 回归与文档收口
  - [x] 更新 docs 中协作、changelog 生命周期、弱网排障说明
  - [x] 完成可用自动验收：编译、JS 语法、日志、HTTP 降级、管理端
  - [x] 整理最终风险与后续建议
  - [ ] 全量关键 UI 流程截图验收：当前本机 Playwright/npm 环境不可用，浏览器控制工具未暴露

- [x] T7 诊断可读性与排障 SOP
  - [x] 放大协作连接诊断弹窗，原始诊断信息默认展开
  - [x] 诊断信息增加同步通道、降级轮询、最近活动、最近错误
  - [x] 新增《BLM协作与弱网排障指南》，说明日志格式、HTTP 降级机制、排查步骤
  - [x] 将排障指南加入内置文档入口
  - [x] 管理端默认端口改为 8091，配置常量放在 `blm.py` 顶部
  - [x] 自动验收：JS 语法检查、Python 编译、配置默认值 smoke test、文档入口 smoke test
