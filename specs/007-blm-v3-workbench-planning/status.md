...
- 等用户验收 T3 全景工作台价值流与业务域视图归位。
- 验收通过后，再清理旧流程工作台里的全景视图入口。
- 后续流程工作台边界收敛为阶段视图、流程视图；其他全局能力归全景工作台。
...
### T3 全景工作台价值流与业务域 Angular 化
- 用户重新确认第一步策略：不在全景视图上增加编辑功能，全景视图继续只做当前查看效果。
- 已在全景工作台新增 `价值流与业务域` tab。
- 已新增 Angular island 挂载机制，由 `ValueDomainWorkbenchComponent` 接管该 tab 内容。
- `价值流与业务域` tab 不再调用旧流程工作台全景视图渲染函数，先覆盖价值流、业务域、阶段归属和矩阵编辑。
- 本轮暂不删除旧流程工作台全景入口，等 Angular 版体验稳定后再清理。
