...
BLM 的基础用户账号只用于协作可见性，不做权限划分：

- `用户`：浏览器本地保存的协作身份，包含稳定 `userId` 和显示名称。
- `连接`：一个浏览器标签页打开一个文档后建立的 WebSocket 会话，包含独立 `sessionId/clientId`。
- 同一浏览器多个标签页共享同一个用户，但会产生多个连接；服务端 presence 按 `userId` 聚合，显示为“张三（2个窗口）”。
- 如果没有设置显示名称，前端提示“用户信息配置”，不再随机生成“用户4962”这类难以识别的名称；配置前不允许打开或编辑文档。
