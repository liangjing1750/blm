...
describe('App', () => {
  ...
  it('should persist a local collab draft when a modified runtime refresh is emitted', async () => {
    vi.stubGlobal('indexedDB', undefined);
    localStorage.clear();
    sessionStorage.setItem('blm.collab.sessionId', 'session-test');
    localStorage.setItem('blm.user.profile', JSON.stringify({ id: 'user-test', name: '测试用户' }));
    const runtime = getAngularRuntimeState();
    runtime.currentFile = 'component.json';
    runtime.readOnly = false;
    runtime.runtime.supportsCollab = true;
    runtime.doc = { meta: { title: '组件草稿' }, businessComponents: [{ uid: 'c1', name: '核心组件' }] };
    runtime.modified = true;
    runtime.collab.pendingSnapshot = true;
    runtime.collab.acceptedSeq = 6;
    ...
    const draft = JSON.parse(localStorage.getItem(`blm.collab.draft.${encodeURIComponent('component.json::user-test')}`) || 'null');
    expect(draft).toMatchObject({
      docName: 'component.json',
      userId: 'user-test',
      baseSeq: 6,
      document: { meta: { title: '组件草稿' } },
    });
  });
  ...
  it('should compare the current document with an archived version of another workspace document', async () => {
    let versionLoadPayload: any = null;
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
      const url = String(input);
      if (url.includes('/api/files/meta')) {
        return new Response(JSON.stringify([
      ...
      if (url.includes('/api/versions/agent-old.json')) {
        return new Response(JSON.stringify([
          { id: 'v-archived', label: '归档验收版', createdAt: '2026-06-24T11:38:19Z' },
        ]), { status: 200, headers: { 'Content-Type': 'application/json' } });
      }
      if (url.includes('/api/version/load')) {
        versionLoadPayload = JSON.parse(String(init?.body || '{}'));
        return new Response(JSON.stringify({
          document: {
            meta: { domain: 'Agent Archived' },
            roles: [{ uid: 'role-1', name: '监管员' }],
            stages: [],
            stageFlowRefs: [],
            processes: [{ uid: 'proc-1', name: '归档入库流程', nodes: [] }],
            entities: [],
            businessComponents: [],
            taskDefinitions: [],
            terms: [],
    ...
    expect(versionLoadPayload).toMatchObject({ name: 'agent-old.json', version_id: 'v-archived' });
    expect(compiled.querySelector('[data-testid="compare-result"]')?.textContent).toContain('归档入库流程');
  ...
  it('should compare an archived version of the current document with another workspace document', async () => {
    let leftVersionLoadPayload: any = null;
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
      const url = String(input);
      if (url.includes('/api/files/meta')) {
        return new Response(JSON.stringify([
      ...
      if (url.includes('/api/version/load')) {
        leftVersionLoadPayload = JSON.parse(String(init?.body || '{}'));
        return new Response(JSON.stringify({
          document: {
            meta: { domain: 'Agent Archived Left' },
            roles: [{ uid: 'role-1', name: '监管员' }],
            stages: [],
            stageFlowRefs: [],
            processes: [{ uid: 'proc-1', name: '左侧归档入库流程', nodes: [] }],
            entities: [],
            businessComponents: [],
            taskDefinitions: [],
            terms: [],
      ...
      if (url.includes('/api/load/agent-old.json')) {
        return new Response(JSON.stringify({
          document: {
            meta: { domain: 'Agent Old Current' },
            roles: [{ uid: 'role-1', name: '监管员' }],
            stages: [],
            stageFlowRefs: [],
            processes: [{ uid: 'proc-1', name: '右侧当前入库流程', nodes: [] }],
            entities: [],
            businessComponents: [],
            taskDefinitions: [],
            terms: [],
    ...
    expect(leftVersionLoadPayload).toMatchObject({ name: 'agent.json', version_id: 'v-left' });
    expect(compiled.querySelector('[data-testid="compare-result"]')?.textContent).toContain('左侧归档入库流程');
  ...
  it('should let users maintain service groups and interfaces inside the selected group', async () => {
    ...
    compiled.querySelector<HTMLButtonElement>('[data-testid="service-group-drawer-save"]')?.click();
    fixture.detectChanges();
