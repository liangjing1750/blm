...
type ToolbarModal = '' | 'open' | 'history' | 'placeholder';
...
export class LegacyShellComponent implements OnInit {
  ...
  protected readonly hasDocument = computed(() => !!this.runtime.currentFile);
  ...
  protected async createDocument(): Promise<void> {
    ...
    await this.runBusy(async () => {
      const created = await this.api.create(name.trim());
      const loaded = created?.document ? created : await this.api.load(name.trim());
      this.openLoadedDocument(name.trim(), loaded);
