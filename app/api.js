...
const api = {
  ...
  aiAskStream(workspace, question, onChunk, onDone, onError) {
    const xhr = new XMLHttpRequest();
    ...
    xhr.onprogress = () => {
      ...
      for (const line of lines) {
        if (line.startsWith('data: ')) {
          ...
          } catch (_) { /* incomplete JSON, wait for more */ }
        }
      }
    };
    xhr.onloadend = () => { onDone && onDone(); };
    xhr.onerror = () => { onError && onError('网络错误'); };
    xhr.send(JSON.stringify({ workspace, question }));
  },

  async aiExportStart(workspace) {
    return postJson('/api/ai/export/start', { workspace });
  },

  async aiExportJob(jobId) {
    return fetch(`/api/ai/export/${encodeURIComponent(jobId)}`).then((response) => response.json());
  },

  async downloadAIExportJob(jobId) {
    return fetch(`/api/ai/export/${encodeURIComponent(jobId)}/download`);
  },

  /** AI 对话 SSE 流式接口 — 传递 userId，接收完整 WireEvent */
  aiChatStream(prompt, onEvent, onDone, onError) {
    const xhr = new XMLHttpRequest();
