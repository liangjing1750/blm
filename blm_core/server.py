...
def create_handler(app_dir: Path, storage: WorkspaceStorage, collab: CollaborationManager | None = None):
    ...
    class BlmRequestHandler(http.server.SimpleHTTPRequestHandler):
        ...
        def _handle_panorama_docx(self, body: bytes):
            """接收全景视图截图，返回 DOCX（按预览大纲结构生成多级标题，含真实数据）。"""
            try:
                payload = self._decode_json(body)
                if isinstance(payload, tuple):
                    return self._json(payload[0], payload[1])
                doc_name = str(payload.get("docName", "") or payload.get("name", "") or "").strip()
                ...
                # 加载文档数据
                try:
                    frozen = storage.load(doc_name) if doc_name else {}
                except Exception:
                    frozen = {}
                stages = frozen.get("stages") or []
                processes = frozen.get("processes") or []
                pan = frozen.get("panorama") or {}
                cols = pan.get("columns") or [{"name": "默认", "uid": "c1"}]
                lanes = pan.get("lanes") or [{"name": "默认", "uid": "l1"}]

                md = f"# 全景视图\n\n"
                md += f"**文档**: {doc_name or '未命名'} | **价值流数**: {len(cols)} | **业务域数**: {len(lanes)} | **阶段**: {len(stages)} | **流程**: {len(processes)}\n\n"
                md += "## 1 全景矩阵\n\n"
                md += f"| 业务域 / 价值流 | " + " | ".join(c.get("name", "") or c.get("uid", "") for c in cols) + " |\n"
                md += "|" + "|".join("---" for _ in cols) + "|\n"

                def lane_uid(l):
                    return str(l.get("uid") or l.get("id") or "")
                def col_uid(c):
                    return str(c.get("uid") or c.get("id") or "")
                def stage_col_uid(s):
                    return str(s.get("panoramaColumnUid") or s.get("columnUid") or "")
                def stage_lane_uid(s):
                    return str(s.get("panoramaLaneUid") or s.get("laneUid") or "")

                for lane in lanes:
                    row = [lane.get("name", "") or lane.get("uid", "")]
                    for col in cols:
                        matched = [s for s in stages if stage_lane_uid(s) == lane_uid(lane) and stage_col_uid(s) == col_uid(col)]
                        row.append(", ".join(s.get("name", "?") for s in matched) if matched else "—")
                    md += "| " + " | ".join(row) + " |\n"
                md += "\n"
                ...
                for ci, col in enumerate(cols, 1):
                    col_stages = [s for s in stages if stage_col_uid(s) == col_uid(col)]
                    if not col_stages:
                        continue
                    md += f"### {ci} {col.get('name','') or col.get('uid','')}\n\n"
                    md += "| 阶段 | 流程数 |\n|------|--------|\n"
                    for s in col_stages:
                        refs = [r for r in (frozen.get("stageFlowRefs") or []) if (r.get("stageUid") or r.get("stageId") or "") == (s.get("uid") or s.get("id") or "")]
                        md += f"| {s.get('name','?')} | {len(refs)} |\n"
                    md += "\n"

                md += "## 3 组件建模\n\n"
                constructs = frozen.get("businessConstructs") or []
                for c in constructs:
                    md += f"### 构件：{c.get('name','未命名')}\n\n"
                    entities = [e for e in (frozen.get("entities") or []) if e.get("businessConstructUid") == c.get("uid")]
                    for e in entities:
                        md += f"- **实体：{e.get('name','?')}**\n"
                    tasks = [t for t in (frozen.get("taskDefinitions") or []) if t.get("businessConstructUid") == c.get("uid") or t.get("constructUid") == c.get("uid")]
                    for t in tasks:
                        md += f"- **任务：{t.get('name','?')}** 地址：{t.get('address','—')}\n"
                    md += "\n"

                from blm_core.docx import DocxImage, build_docx_with_screenshots
                img = DocxImage(name="panorama.png", content_type="image/png", payload=raw, width=1200, height=800)
                docx_bytes = build_docx_with_screenshots(md, title=doc_name or "全景视图", screenshots=[img])
                return self._binary(
                    docx_bytes,
                    "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                    filename=f"{doc_name or 'panorama'}.docx",
                )
            except Exception as exc:
                import traceback
                traceback.print_exc()
