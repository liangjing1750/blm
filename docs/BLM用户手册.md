...
V3 版本开始，BLM 会把“构件能力”和“应用服务编排”分开维护：

- 构件工作台：维护业务组件、业务构件、构件任务、任务输入输出和技术承接。
- 应用编排工作台：维护应用服务接口、请求参数、返回参数、编排步骤和参数变量映射。
- 流程工作台：在节点详情中回看关联应用服务，但不维护编排细节。

#### 构件任务

构件任务表示某个业务构件能提供的抽象能力，例如“校验仓库状态”“保存入库预约”“生成待办”。编辑构件任务时，可以维护：

- 任务名称和任务类型
- 所属构件
- 输入参数和输出参数
- 技术承接说明，包括运行承接类型、目标标识和备注

技术承接只是设计说明，不代表当前 BLM 会直接执行该任务。它用于让后端研发和后续 Agent 工具理解这个能力将来可能落到聚合根方法、领域服务、查询接口或外部系统调用。

#### 应用服务

应用服务表示无状态接入服务对外暴露的接口，例如 `POST /inbound-reservations/submit`。在应用编排工作台中，可以维护：
...
当前“前端接口需求”和“应用服务”不拆成两个概念，统一在应用服务里表达。这样一个页面提交动作或流程节点动作，可以先被定义成清晰的服务入口，再继续编排构件任务。

#### 参数变量映射

应用服务的变量只在一次服务调用内有效，不做全局变量。第一版可录入以下变量路径：

- `request.*`：来自应用服务请求参数
- `context.*`：来自调用上下文
- `step.<stepAlias>.*`：来自前面步骤的输出绑定
- `const.*`：应用服务内部局部常量
- `return.*`：返回结果组装目标

使用时先在应用服务下添加编排步骤，每个步骤选择一个构件任务，并维护稳定别名。随后为步骤输入填写来源变量，为步骤输出填写绑定目标，最后用返回映射把步骤变量组装成服务返回参数。

#### 流程节点回看应用服务

流程节点详情中的“应用服务”区域只用于回看当前节点关联了哪些接口，展示名称、方法、路径和说明。产品经理不需要在这里维护参数映射、构件任务输入输出或技术承接。
